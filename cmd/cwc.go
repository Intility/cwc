...
func createInteractiveCommand(args []string, opts internal.InteractiveChatOptions) *internal.InteractiveCmd {
	...
	retrieverConfig := systemcontext.FileContextRetrieverOptions{
		...
		ContextPrinter: func(fileTree string, files []filetree.File) {
			printContext(fileTree, files)
			printToolsEnabled(opts.ToolsEnabled)
		},
	}
...
func printToolsEnabled(toolsEnabled []string) {
	if len(toolsEnabled) > 0 {
		toolsEnabledMsg := fmt.Sprintf("Enabled tools: %v\n", toolsEnabled)
		ui.PrintMessage(toolsEnabledMsg, ui.MessageTypeInfo)
	}
}
